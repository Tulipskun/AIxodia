...
private suspend fun route(request: dynamic, env: dynamic): dynamic {
    ...
    if (path == "/api/node" && method == "GET") {
        ...
        val age = if (row == null) -1L else now - (row.heartbeat as Number).toLong()
        return json(dynamic {
    ...
    val revoke = matchPath(path, "/api/devices/")
    if (revoke != null && revoke.endsWith("/revoke") && method == "POST") {
        db.prepare("UPDATE devices SET revoked = 1 WHERE id = ?")
            .bind(decodeURIComponent(revoke.removeSuffix("/revoke"))).run()
        return json(dynamic { ok = true })
    }
    ...
    if (turns != null) {
        ...
        if (method == "POST") {
            ...
            val max = db.prepare("SELECT COALESCE(MAX(seq),0) AS m FROM turns WHERE session_id = ?")
                .bind(sid).first()
            val seq = (((max?.m as Number?)?.toDouble() ?: 0.0) + 1.0).toLong()
...
private fun changed(info: dynamic): Boolean =
    ((info?.meta?.changes as Number?)?.toDouble() ?: 0.0) > 0.0
